...
const virtual = function(z_real, w_copy={}) {
	...
	// virtual copy needs to undefine name
	if(h_undefine.name) {

		// name is a weird one; first define with writable setting false
		h_undefine.name.writable = false;
	}
	...
	// virtual copy redefined name
	if(h_undefine.name) {

		// now, redefine name again with writable setting true
		h_undefine.name.writable = true;
		Object.defineProperty(w_copy, 'name', h_undefine.name);
	}
